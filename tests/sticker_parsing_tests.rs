...
fn test_7099() {
    let s =
        Sticker::parse_stickers("7099_LRS_НЕЖЕН САПУН ОБОГАТЕН С МАСЛА_60X40_PVC_R (2)").unwrap();
...
#[test]
fn test_7303() {
    let s = Sticker::parse_stickers("7303_LRS_SOS ВЪЗСТАНОВЯВАЩ БАЛСАМ БЕБЕ 50X30_PVC_R_TEMP_SIZE")
        .unwrap();
...
fn test_7396() {
    let s =
        Sticker::parse_stickers("7396_LRS_НЕЖЕН ИЗМИВАЩ ГЕЛ БЕБЕ ПЪЛНИТЕЛ 80X55_PVC_R").unwrap();
    assert_eq!(s[0].dimensions, "80x55".parse::<Dimensions>().unwrap());
...
#[test]
fn test_234191() {
    let s = Sticker::parse_stickers("234191_AV CLEAN GEL TUBE 200ML_50X50_PVC_R_OK_PF").unwrap();
...
fn test_235354() {
    let s =
        Sticker::parse_stickers("235354_RF STYLE FIX GEL 150ML_45X101_PVC_R_SLV_OK_PF").unwrap();
...
fn test_237355() {
    let s =
        Sticker::parse_stickers("237355_AD DERMALIB CICA CR REP 50ML_100X40_PVC_OK_PF").unwrap();
...
#[test]
fn test_239680() {
    let s = Sticker::parse_stickers("239680_AD EPITHELIALE AH MASSAGE OIL 100ML_45X101_PVC_OK_PF")
        .unwrap();
    assert_eq!(s[0].dimensions, "45x101".parse::<Dimensions>().unwrap());
...
#[test]
fn test_240198() {
    let s = Sticker::parse_stickers("240198_KL MASQUE REPA CAPUACY 3IN1 150ML_60X40_PVC_R_OK_PF")
        .unwrap();
    assert_eq!(s[0].dimensions, "60x40".parse::<Dimensions>().unwrap());
...
fn test_241438() {
    let s =
        Sticker::parse_stickers("241438_DU KERTYOL PSO SHP 125ML_50X100_PAPER BLUE_OK_PF").unwrap();
...
fn test_253831() {
    let s =
        Sticker::parse_stickers("253831_AD BIOLOGY HIALU SER 3IN1 30ML_40X90_PAPER(GR)BLK_OK_PF")
            .unwrap();
...
fn test_259839() {
    let s =
        Sticker::parse_stickers("259839_RF ABSOLU KERATIN CR 100ML_40X100_PAPER BLK_PF").unwrap();
...
fn test_261783() {
    let s =
        Sticker::parse_stickers("261783_AV VITAMIN ACTIV CG SER 30ML_40X45_PVC_R_OK_PF").unwrap();
...
#[test]
fn test_263673_gel() {
    let s = Sticker::parse_stickers("263673_ELGY CLINIC SENSILEAVE GEL - TUBE_40X20_PVC_R_OK_PF")
        .unwrap();
...
fn test_263673_box() {
    let s =
        Sticker::parse_stickers("263673_ELGY CLINIC SENSILEAVE GEL - BOX_121X27_PAPER BLUE_OK_PF")
            .unwrap();
...
fn test_269226() {
    let s =
        Sticker::parse_stickers("269226_ULTRA FLUIDE_RADIANCE_80X25_PVC_REGULJAREN_OK").unwrap();
...
fn test_268175() {
    let s =
        Sticker::parse_stickers("268175_DU KELUAL SQUANORM OILY SHP 200ML_50X30_PVC-R").unwrap();
...
fn test_infer_with_one_typo() {
    let existing =
        Sticker::parse_stickers("234191_AV CLEAN GEL TUBE 200ML + RANDOM LENGHT_50X50_PVC_R_OK_PF")
            .unwrap();
...
fn test_infer_with_missing_character() {
    let existing =
        Sticker::parse_stickers("234191_AV CLEAN GEL TUBE 200ML + RANDOM LENGTH_50X50_PVC_R_OK_PF")
            .unwrap();
    let error = ParseStickerError::MissingCode(
